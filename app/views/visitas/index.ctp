<?php
	...
	print $this->Form->create( 'Visita', array( 'url' => array( 'controller' => 'visitas', 'action' => 'index' ), 'class' => 'form' ) );
?>
...
<table class="visualizar medium">
	<tr>
		...
		<td><?= $this->Form->input( 'Visita.vendedor_id', array( 'label' => false, 'escape' => false, 'type' => 'select', 'class' => 'select', 'empty' => '--', 'options' => $vendedores ) ) ?></td>
	</tr>
</table>
...
<?php if( isset( $visitas ) ): ?>
	...
	<?php if( empty( $visitas ) ){ ?>
		...
		<p class="bold">Não foram encontradas visitas nestas datas para este vendedor.</p>
	...
	<?php } else { ?>
		...
		$(document).ready( function(){
			...
			var vendedorLocation = "<?= $vendedorLocalizacao ?>";
